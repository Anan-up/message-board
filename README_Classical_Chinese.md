...
> 一份完备之部署指南，含源码、配置之说明及常见疑难之排解

## 📌 引言

方今互联之世，留言板犹为极具价值之交互之器。无论个人博客、小型社群，抑或企业内之通联，一简洁而功能完备之留言板，皆能大展其用。

本文将引君自零而始，于 Linux 服务器之上部署一功能丰赡之留言板网站。此项目取 **Nginx + PHP + SQLite** 之轻量架构，无须另装 MySQL，所耗资源甚微，宜部署于云服务器，乃至树莓派之上。
...
| 组件 | 版本 | 说明 |
|------|------|------|
| 操作系统 | Debian 13.2 | 亦可兼容 Ubuntu 22.04+ |
| Web 服务器 | Nginx 1.24+ | 高性能 HTTP 服务 |
| PHP | PHP 8.3 / 8.4 | 后端逻辑之处理 |
| 数据库 | SQLite 3 | 轻量级文件数据库 |
| 前端 | 原生 HTML + CSS + JavaScript | 无须任何框架 |
...
- 一个已解析至服务器 IP 之域名（用于 HTTPS）
- SSL 证书（本教程用已有证书，亦可用 Let's Encrypt 免费签发）

---

## 🚀 第一步：环境准备

登录服务器后，先更新系统，并安装所需之软件包：
...
> **说明**：`your_domain` 请易为汝之实际域名。

---

## 🔐 第三步：准备 SSL 证书（可选而推荐）

若已有证书文件（`.crt` 与 `.key`），置之服务器上（如 `/path/to/ssl/`）。若无，可用 Let's Encrypt 免费签发：
...
此乃项目之核心文件，留言板之一切功能尽在其中。存之为 `/var/www/your_domain/index.php`：
...
> **注意**：务必改 `define('ADMIN_PASSWORD', 'your_password_here');` 中之密码为强密码。
...
确保文件名为 `background.mp3`，代码中之播放器自会加载。
...
1. 发布一条留言（带或不带附件）
2. 点按点赞之钮（长按可连赞，兼有爱心之效）
3. 切换「最新」「热门」之排序
4. 管理员登录（点右上角「管理员」，输入密码）
5. 置顶或删除留言
6. 刷新页面，观「每日一言」之变化
7. 观访客统计
...
**原因**：Nginx 或 PHP 上传大小之限不足。

**解决**：查 Nginx 配置中之 `client_max_body_size` 与 PHP 配置中之 `upload_max_filesize`、`post_max_size`，确保足够大，然后重启服务。

### Q2：留言中之图片/视频不显示

**原因**：上传目录权限不当，或 Nginx 未正确访问。
...
**解决**：于 `index.php` 中改 `define('ADMIN_PASSWORD', 'new_password');`，然后刷新页面。
...
1. **开启 Nginx 缓存**：为静态资源（图片、CSS、JS）设缓存头。
2. **用 Redis 缓存**：若有高并发之需，可引入 Redis 缓存热门留言列表。
3. **定期清理上传目录**：过期或无关联之附件，可写脚本定期清理。
4. **启用 Gzip 压缩**：减传输之体积，增加载之速。

---

## 🎯 总结

至此，君已成功部署一功能完备之留言板网站。此项目具以下特点：

- ✅ **轻量级**：基于 SQLite，无须额外数据库服务
- ✅ **功能丰富**：含点赞、置顶、排序、附件上传、访客统计等
- ✅ **安全可靠**：支持 HTTPS，上传文件类型受限，防注入之处理
- ✅ **响应式设计**：适配桌面与移动设备
- ✅ **易于维护**：单文件架构，便于修改与扩展
